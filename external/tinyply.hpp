...
public:
    ...
    Buffer(uint8_t* ptr)
    { alias = ptr; } // non-allocating, todo: set size?
    uint8_t* get()
    { return alias; }
...
inline T2 endian_swap(const T& v)
{ return v; }

template <>
inline uint16_t endian_swap<uint16_t, uint16_t>(const uint16_t& v)
{ return (v << 8) | (v >> 8); }

template <>
inline uint32_t endian_swap<uint32_t, uint32_t>(const uint32_t& v)
{ return (v << 24) | ((v << 8) & 0x00ff0000) | ((v >> 8) & 0x0000ff00) | (v >> 24); }

template <>
inline uint64_t endian_swap<uint64_t, uint64_t>(const uint64_t& v)
{
...
template <>
inline int16_t endian_swap<int16_t, int16_t>(const int16_t& v)
{
...
template <>
inline int32_t endian_swap<int32_t, int32_t>(const int32_t& v)
{
...
template <>
inline int64_t endian_swap<int64_t, int64_t>(const int64_t& v)
{
...
template <>
inline float endian_swap<uint32_t, float>(const uint32_t& v)
{
    union
    {
        float f;
...
template <>
inline double endian_swap<uint64_t, double>(const uint64_t& v)
{
    union
    {
        double d;
...
struct PlyFile::PlyFileImpl
{
    ...
    void add_properties_to_element(const std::string& elementKey,
                                   const std::initializer_list<std::string> propertyKeys,
                                   const Type type, const size_t count, uint8_t* data, const Type listType,
                                   const size_t listCount);
...
void
PlyFile::PlyFileImpl::read_header_text(std::string line, std::istream& is, std::vector<std::string>& place, int erase)
{
...
size_t PlyFile::PlyFileImpl::read_property_binary(const Type& t, const size_t& stride, void* dest, size_t& destOffset,
                                                  std::istream& is)
{
    destOffset += stride;
...
size_t PlyFile::PlyFileImpl::read_property_ascii(const Type& t, const size_t& stride, void* dest, size_t& destOffset,
                                                 std::istream& is)
{
    destOffset += stride;
    switch (t)
        ...
        for (auto& entry : userData)
        {
            if (entry.second.data == b && b->buffer.get() == nullptr)
            {
                ...
                else
                {
                    ...
                    auto bytes_per_property =
                        entry.second.data->count * PropertyTable[entry.second.data->t].stride * list_size_multiplier;
        ...
        for (size_t i = 0; i < e.size; ++i)
        {
            ...
            for (auto& p : e.properties)
            {
                ...
                if (p.isList)
                {
                    ...
                    write_property_binary(p.propertyType, os, (helper->data->buffer.get() + helper->cursor->byteOffset),
    ...
    for (auto& e : elements)
    {
        for (size_t i = 0; i < e.size; ++i)
        {
            for (auto& p : e.properties)
            {
                ...
                {
                    write_property_ascii(p.propertyType, os, (helper.data->buffer.get() + helper.cursor->byteOffset),
                                         helper.cursor->byteOffset);
                }
            }
            os << "\n";
    ...
    if (elementIndex >= 0)
    {
        ...
        for (auto key : propertyKeys)
        {
            ...
            if (propertyIndex >= 0)
            {
                ...
                auto result = userData.insert(
                    std::pair<uint32_t, ParsingHelper>(hash_fnv1a(element.name + property.name), helper));
                if (result.second == false)
                {
                    throw std::invalid_argument(
                        "element-property key has already been requested: " + hash_fnv1a(element.name + property.name));
                }
            }
...
void PlyFile::PlyFileImpl::add_properties_to_element(const std::string& elementKey,
                                                     const std::initializer_list<std::string> propertyKeys,
                                                     const Type type, const size_t count, uint8_t* data,
                                                     const Type listType, const size_t listCount)
{
    ParsingHelper helper;
    helper.data = std::make_shared<PlyData>();
    ...
    auto create_property_on_element = [&](PlyElement& e)
    {
        for (auto key : propertyKeys)
        {
            PlyProperty newProp = (listType == Type::INVALID) ? PlyProperty(type, key) : PlyProperty(listType, type,
                                                                                                     key, listCount);
    ...
    {
        PlyElement newElement = (listType == Type::INVALID) ? PlyElement(elementKey, count) : PlyElement(elementKey,
                                                                                                         count);
...
{
    std::function<void(PropertyLookup& f, const PlyProperty& p, uint8_t* dest, size_t& destOffset,
                       std::istream& is)> read;
    ...
    auto read_list_binary = [this](const Type& t, void* dst, size_t& destOffset, std::istream& _is)
    {
        ...
        if (isBigEndian)
        {
            ...
            {
                case Type::INT16:
                    endian_swap<int16_t, int16_t>(*(int16_t*) dst);
                    break;
                case Type::UINT16:
                    endian_swap<uint16_t, uint16_t>(*(uint16_t*) dst);
                    break;
                case Type::INT32:
                    endian_swap<int32_t, int32_t>(*(int32_t*) dst);
                    break;
                case Type::UINT32:
                    endian_swap<uint32_t, uint32_t>(*(uint32_t*) dst);
                    break;
            }
        }
    ...
    if (isBinary)
    {
        read = [this, &listSize, &dummyCount, &read_list_binary](PropertyLookup& f, const PlyProperty& p, uint8_t* dest,
                                                                 size_t& destOffset, std::istream& _is)
        {
            if (!p.isList)
            {
            ...
            else
            {
                ...
                read_property_binary(p.propertyType, f.prop_stride * listSize, dest + destOffset, destOffset,
                                     _is); // properties in list
            }
        };
        skip = [this, &listSize, &dummyCount, &read_list_binary](PropertyLookup& f, const PlyProperty& p,
                                                                 std::istream& _is)
        {
            if (!p.isList)
            {
    ...
    {
        read = [this, &listSize, &dummyCount](PropertyLookup& f, const PlyProperty& p, uint8_t* dest,
                                              size_t& destOffset, std::istream& _is)
        {
            if (!p.isList)
            {
        ...
        skip = [this, &listSize, &dummyCount, &skip_ascii_buffer](PropertyLookup& f, const PlyProperty& p,
                                                                  std::istream& _is)
        {
...
std::vector<PlyElement> PlyFile::get_elements() const
{ return impl->elements; }

std::vector<std::string>& PlyFile::get_comments()
{ return impl->comments; }

std::vector<std::string> PlyFile::get_info() const
{ return impl->objInfo; }
